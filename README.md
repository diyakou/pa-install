...
# نکته مهم
جهت نصب به ایدی تلگرام پیام بدید دسترسی تون برای کرنل باز بشه 
**Telegram : EXCHI**
...
7. حین نصب، مقدار `حداکثر کاربران آنلاین همزمان` گرفته می‌شود و تنظیمات اتصال به‌صورت هوشمند اعمال می‌شود.
...
- یا `Port Forward`
6. اسکریپت از شما **حداکثر کاربران آنلاین همزمان** را می‌پرسد و تنظیمات اتصال را هوشمند اعمال می‌کند.
7. در ادامه می‌توانید انتخاب کنید:
- بهینه‌سازی Kernel/OS برای تانل اعمال شود
- کرنل سیستم هم آپگرید شود (در صورت پشتیبانی توزیع)
- قبل از استارت سرویس، بخش اتصال کانفیگ ساخته‌شده را ویرایش کنید
8. در پایان سرویس ساخته و اجرا می‌شود.
...
- در نسخه جدید، اسکریپت به‌جای `fast/fast2/fast3` از پروفایل هوشمند `manual` استفاده می‌کند.
- فقط تعداد **کاربر آنلاین همزمان** را وارد کنید؛ مقدارهای `conn`, `mtu`, `kcp window`, `buffer` خودکار تنظیم می‌شوند.
- علاوه بر تعداد کاربر، منابع واقعی سیستم (`CPU/RAM`) هم بررسی می‌شود و خروجی نهایی با ظرفیت سرور/کلاینت تطبیق داده می‌شود.
- اگر از منوی `Kernel Optimization` استفاده کنید، بعد از اعمال تنظیمات کرنل می‌تواند کانفیگ‌های موجود را هم با سؤال تعداد یوزر آنلاین به‌صورت هوشمند retune و سرویس‌ها را restart کند.
- در صورت نیاز، هنگام نصب می‌توانید قبل از اجرا فایل کانفیگ نهایی را هم دستی ویرایش کنید.
